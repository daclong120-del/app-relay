 ...
 * pull-from-play.ts
 * 
 * Standalone automated pipeline script to pull APK + Play listing from Google Play via emulator.
 * Implements the full spec from pull-from-play (3).md.
 * 
 * Usage:
 *   npx tsx scripts/pull-from-play.ts <play_url_1> [play_url_2 ...]
 * 
 * Example:
...
async function triggerPlayStoreInstall(packageId: string): Promise<boolean> {
  ...
  // Force stop and launch market URL
  ...
  // Dump UI XML to inspect for Install button
  ...
  try {
    ...
    console.warn(`  ⚠️ Could not dump UI XML via uiautomator: ${err}`);
  }
  ...
  if (existsSync(dumpPath)) {
    ...
    // Look for Install or Cài đặt node bounds
    ...
    if (installMatch) {
      ...
      console.log(`  → Install text node not exact match. Tapping typical Install button location (850, 780)...`);
      execAdb('shell input tap 850 780');
    }
  } else {
    // Fallback tap if dump failed
    console.log(`  → XML dump missing. Tapping estimated Install coordinates...`);
    execAdb('shell input tap 850 780');
  }
...
async function processPackageUrl(playUrl: string) {
  const urlObj = new URL(playUrl);
  const packageId = urlObj.searchParams.get('id');

  if (!packageId) {
    throw new Error(`Invalid Play Store URL (missing ?id= parameter): ${playUrl}`);
  }
  ...
  // ── Step 1: Scrape Play Store Listing ────────────────────────────────
  console.log(`\n[STEP 1/5] Scraping Play Store listing...`);
  const scrapeResult = await scrapePlayListingOneOff(playUrl, pkgWorkDir, {
    downloadAssets: true,
  });
  ...
  // ── Step 2: Check Emulator Environment ──────────────────────────────
  console.log(`\n[STEP 2/5] Checking Emulator environment...`);
  const devicesRaw = execAdb('devices');
  if (!devicesRaw.includes('device')) {
    throw new Error('No active ADB device detected. Ensure emulator (chpay) is booted.');
  }

  // Wakeup & unlock emulator screen
  execAdb('shell input keyevent KEYCODE_WAKEUP');
  execAdb('shell input keyevent 82');

  // ── Step 3: Install app from Play Store (if not installed) ───────────
  ...
  if (baseStat.size < 1000) {
    throw new Error(`Verification failed: base.apk is too small (${baseStat.size} bytes)`);
  }

  console.log(`\n════════════════════════════════════════════════════════════`);
  console.log(`  SUCCESS — PULL COMPLETED FOR ${packageId}`);
  console.log(`════════════════════════════════════════════════════════════`);
...
async function main() {
  ...
  const urls = args.length > 0 ? args : [
    'https://play.google.com/store/apps/details?id=colorwidgets.ios.widget.topwidgets&hl=en',
  ];

  for (const url of urls) {
    try {
      await processPackageUrl(url);
    } catch (err: any) {
